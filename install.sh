...
sudo apt-key adv --keyserver keyserver.ubuntu.com --recv-keys FF389421CBC8B383
sudo apt-add-repository "deb [arch=amd64] http://repo.powerdns.com/ubuntu trusty-auth-master main"
...
# uncompress
tar -xzvf /var/www/html/t/poweradmin-2.1.7.tgz
...
sudo cp /vagrant/pdns.conf /etc/powerdns/ && sudo service pdns restart
